...
data = pickle.load(open("sample_pd_scale-1.0_DC-0_TH-1_Balanced-1_dur-1.pkl","rb"))
cfg.data = data
...
for i,(pop, sz) in enumerate(zip(L, N_Full)):
    for idx in range(sz):
        netParams.popParams[f"{pop}_{idx}"] = {'cellType': 'SC', 'numCells': 1, 'cellModel': pop, 
        'xRange': [-cfg.borderX[0], cfg.sizeX - cfg.borderX[1]],
        'yRange' : [-cfg.borderY[0] + popDepths[i][0] * cfg.sizeY, cfg.sizeY * popDepths[i][1] - cfg.borderY[1]],
...
for pop in L:
    cellRule = netParams.importCellParams(label='cellRule', fileName='Neuron.py', 
                    conds={'cellType' : 'SC', 'cellModel' : pop}, cellName=pop)
...
regions['mem'] = {'cells' : all_cells, 'secs' : 'all', 'nrn_region' : None, 'geometry' : 'membrane'}
...
species['kki'] = {'regions' : ['cyt'], 'd' : 2.62, 'charge' : 1, 'name':'k',
                'initial' : constants['ki_initial']}
species['nai'] = {'regions' : ['cyt'], 'd' : 1.78, 'charge' : 1, 'name':'na',
                'initial' : constants['nai_initial']}

species['cli'] = {'regions' : ['cyt'], 'd' : 2.1, 'charge' : -1, 'name':'cl',
                'initial' : constants['cli_initial']}

### parameters
params = {}
params['o2_extracellular'] = {'regions' : ['ecs_o2'], 'initial' : 1e12} #constants['o2_bath']}
params['kko'] = {'regions' : ['ecs'], 'charge' : 1, 'name':'k',
                'value' : constants['ko_initial']}
params['nao'] = {'regions' : ['ecs'], 'charge' : 1, 'name':'na',
                'value' : constants['nao_initial']}

params['clo'] = {'regions' : ['ecs'], 'charge' : -1, 'name':'cl',
...
gliapump = "(1.0/3.0) * (%s / (1.0 + rxd.rxdmath.exp((25.0 - gnai_initial) / 3.0))) * (1.0 / (1.0 + rxd.rxdmath.exp(3.5 - kko[ecs]/vol_ratio[ecs])))" % (p)
g_glia = "g_gliamax / (1.0 + rxd.rxdmath.exp(-((%s)*alpha/vol_ratio[ecs] - 2.5)/0.2))" % (o2ecs)
glia12 = "(%s) / (1.0 + rxd.rxdmath.exp((18.0 - kko[ecs] / vol_ratio[ecs])/2.5))" % (g_glia)
...
# # CURRENTS/LEAKS ----------------------------------------------------------------
# sodium (Na) current
mcReactions['na_current'] = {'reactant' : 'nai[cyt]', 'product' : 'nao[ecs]', 
                            'rate_f' : "%s * (rxd.v - %s )" % (gna, ena),
                            'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

# potassium (K) current
mcReactions['k_current'] = {'reactant' : 'kki[cyt]', 'product' : 'kko[ecs]', 
                            'rate_f' : "%s * (rxd.v - %s)" % (gk, ek), 
                            'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

# nkcc1 (Na+/K+/2Cl- cotransporter)
mcReactions['nkcc1_current1'] = {'reactant': 'cli[cyt]', 'product': 'clo[ecs]', 
                                'rate_f': "2.0 * (%s) * (%s)" % (nkcc1, volume_scale), 
                                'membrane': 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

mcReactions['nkcc1_current2'] = {'reactant': 'kki[cyt]', 'product': 'kko[ecs]',
                                'rate_f': "%s * %s" % (nkcc1, volume_scale), 
                                'membrane': 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

mcReactions['nkcc1_current3'] = {'reactant': 'nai[cyt]', 'product': 'nao[ecs]', 
                                'rate_f': "%s * %s" % (nkcc1, volume_scale), 
                                'membrane': 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

# ## kcc2 (K+/Cl- cotransporter)
mcReactions['kcc2_current1'] = {'reactant' : 'cli[cyt]', 'product': 'clo[ecs]', 
                                'rate_f': "%s * %s" % (kcc2, volume_scale), 
                                'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

mcReactions['kcc2_current2'] = {'reactant' : 'kki[cyt]', 'product' : 'kko[ecs]', 
                                'rate_f': "%s * %s" % (kcc2, volume_scale), 
                                'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}
## sodium leak
mcReactions['na_leak'] = {'reactant' : 'nai[cyt]', 'product' : 'nao[ecs]', 
                        'rate_f' : "gnabar_l * (rxd.v - %s)" % (ena), 
                        'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

# ## potassium leak
mcReactions['k_leak'] = {'reactant' : 'kki[cyt]', 'product' : 'kko[ecs]', 
                        'rate_f' : "gkbar_l * (rxd.v - %s)" % (ek), 
                        'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

# ## chlorine (Cl) leak 
mcReactions['cl_current'] = {'reactant' : 'cli[cyt]', 'product' : 'clo[ecs]', 
                            'rate_f' : "gclbar_l * (%s - rxd.v)" % (ecl), 
                            'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True} 

# ## Na+/K+ pump current in neuron (2K+ in, 3Na+ out)
mcReactions['pump_current'] = {'reactant' : 'kki[cyt]', 'product' : 'kko[ecs]', 
                            'rate_f' : "(-2.0 * %s * %s)" % (pump, volume_scale), 
                            'membrane' : 'mem', 'custom_dynamics' : True, 'membrane_flux' : True}

mcReactions['pump_current_na'] = {'reactant' : 'nai[cyt]', 'product' : 'nao[ecs]', 
